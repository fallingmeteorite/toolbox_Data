...
def Thread_calls(process_src, process_dst, process_number):
    try:
    ...
    for index in range(int(cnt)):
        number += 1
        _processes = (Thread(target=start_run, args=(number, process_dst)))
        _processes.start()

def start_run(number, process_dst):
    time1 = time.time()
    new_file_path = ("../../Cache/Cache_thread/")
    dirct = new_file_path
    dirList = []
